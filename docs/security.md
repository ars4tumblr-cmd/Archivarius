...
## Corporate Security by Architecture вЂ” Not by Policy
...
### 1. The System Is a Local Search Engine вЂ” Not a Cloud AI Service

Archivarius is not a SaaS product, not a cloud service, and not an AI assistant that connects to external models. It is **on-premise search infrastructure** вЂ” all components run on your corporate server.
...
```
[Documents] в†’ [Ingestion] в†’ [Indexing] в†’ [Search] в†’ [User]
     в†‘                                                  в†“
     в””в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ Everything inside corporate network в”Ђв”
...
| Query logs | `./data/logs/` вЂ” SHA-256 hash only, no query text | вќЊ Never |
| Feedback signals | `./data/feedback/` вЂ” hashed IDs only | вќЊ Never |
| AI model | `./models/` вЂ” downloaded once during setup | вќЊ Never |
...
| Secret management | Environment variables вЂ” not stored in version control |
| Admin endpoints | Recommended: intranet/VPN allowlist |
| File sandbox | MIME type check + extension denylist + file size limit |
| Network | Docker internal network for Qdrant and LibreTranslate вЂ” not exposed externally |
...
|-------------|---------------|
| Components available for inspection | All open source вЂ” Qdrant, FastAPI, rank_bm25 |
| Versions pinned | Pinned versions in `requirements.txt` and `docker-compose.yml` |
| SBOM | Software Bill of Materials вЂ” a complete list of every software component and its exact version. Ready to hand over to IT for security review. |
| Licence inventory | MIT, Apache 2.0, EUPL вЂ” all permit commercial/corporate use without restriction |
| Vulnerability scan | Performed before production rollout |
| AI model | multilingual-e5-large by Microsoft Research (MIT licence) вЂ” downloaded with checksum verification, reproducible |
...
| Right to erasure | `delete_document` API в†’ clears all derived indices | Procedure in runbook |
...
✅ **GDPR ready:** by architecture вЂ” no additional measures required
